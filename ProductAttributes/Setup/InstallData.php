...
namespace Codelegacy\ProductAttributes\Setup;
...
{
    const FIXTURE_FILE_PATH = 'Codelegacy_ProductAttributes::importsource/attributes.csv';
    ...
    {
        $entityTypeId = $this->getEntityTypeId(ProductAttributeInterface::ENTITY_TYPE_CODE);
    ...
    private function addAttributesOptions()
    {
        array_map(function ($attributeInfo) {
            if (!empty($attributeInfo['options'])) {
                $this->addOptionsToAttribute(
                    $attributeInfo['attribute_code'], 
     ...
     * @param array $options
     */
    private function addOptionsToAttribute(
        $attributeCode, 
        $options
    ) {
